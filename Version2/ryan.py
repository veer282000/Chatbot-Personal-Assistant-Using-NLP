...
from classify_intent import filename
import pickle
from classify_intent import cv
...
def main():
    ...
    while True:
        ...
        if button in (None, 'Exit'):
            ...
            speak(response)
            statement = takeCommand().lower()
            data = cv.transform([statement]).toarray()
            ...
            elif output== 'wikipedia':
                try:
                    response = random.choice(responses[output])
                    print(response)
                    speak(response)
            ...
            sg.popup('About:', 'Created by Veer Kumar, Asha Gaud, Dhananjay Kumbhar and Bhumi Mota', 'Virtual Assistant:' + '\n\n' + 'Ryan', 'Version 1.0')
